# request-thinking-config-logging Specification

## Purpose
Define how explicit request-side thinking or reasoning configuration is extracted, normalized, persisted, and returned in request logs.

## Requirements
### Requirement: 系统必须从请求体提取显式的 thinking 或 reasoning 配置
The system MUST extract explicit thinking or reasoning configuration from supported JSON request bodies and normalize it into a unified request log configuration object. 系统在处理支持的 JSON 请求协议时，必须从请求体中提取显式传入的 thinking 或 reasoning 配置，并将其归一化为统一的请求日志配置对象。系统不得依赖响应 usage、响应摘要或 SSE 事件去反推该配置。
...
The system MUST return an empty configuration when the request body is not valid JSON, fields are not explicitly specified, or the protocol does not support thinking configuration. 系统在请求体不是有效 JSON、字段未显式指定或协议不支持 thinking 配置时，必须返回空配置，而不是写入推断出的默认等级或预算。
...
The system MUST persist normalized thinking configuration as part of request logs and return it through the admin logs API for frontend display. 系统必须将归一化后的 thinking 配置作为请求日志的一部分持久化，并通过管理端日志 API 返回给前端。历史记录和空值记录必须保持向后兼容。
