...
## Purpose
管理员可在同一工作区管理 CLIProxyAPI 实例、OAuth 账号和关联上游，并查看账号用量与实例日志。
...
管理页面 SHALL 以实例选择和所选实例的账号用量为主工作区，关联上游与实例日志为可切换的次级视图。实例创建、启停、编辑、检测、删除、账号登录、同步、上传和账号行操作 MUST 保持可用。桌面和窄屏布局 MUST 使当前实例、当前视图和主要操作可辨认，关键账号信息无需横向裁切即可读取。
