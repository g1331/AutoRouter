# cliproxy-oauth-pool-upstream Specification

## Purpose
TBD - created by archiving change cliproxy-oauth-pool-upstream. Update Purpose after archive.
## Requirements
...
系统 SHALL 提供将单个 OAuth 账号固定映射为一个 AutoRouter 上游的能力。映射 MUST 为目标账号确定一个账号前缀，账号尚无前缀时系统 SHALL 通过 CLIProxyAPI 为该账号写入前缀。系统 SHALL 以对应服务商的池上游配置为基础创建上游，并记录其绑定的 OAuth 账号文件名。发往单账号映射上游的请求 MUST 由代理在转发前将模型名改写为携带该账号前缀的形式，使 CLIProxyAPI 将请求固定路由到该账号；普通上游与池上游的请求 MUST 不受影响。

#### Scenario: 映射已有前缀的账号

- **WHEN** 管理员将一个已设置前缀的 OAuth 账号映射为上游
- **THEN** 系统沿用该账号已有前缀创建单账号上游
...
- **WHEN** 一个请求经路由选定某单账号映射上游
- **THEN** 代理在转发前将请求中的模型名改写为携带该账号前缀的形式，发往 CLIProxyAPI
- **AND** 选定的是普通上游或池上游时请求体不被改写
