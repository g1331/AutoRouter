...
- 方案：OpenRouter 模型接口为主源，LiteLLM 价格映射为兜底源；保留来源字段与同步时间。
- 原因：OpenRouter 对模型价格结构化程度高，LiteLLM 覆盖面广，双源组合可降低缺失率。
- 备选方案：
  - 仅依赖单一上游官网页面抓取：维护成本高、易失效。
...
- 方案：System/Billing 页面分为四区：概览、倍率、缺失修复、近期明细。
- 原因：用户先看钱，再改配置，再补数据，最后追溯单笔。
...
+--------------------------+-----------------------------------+
| 上游倍率管理              | 缺失价格修复                      |
| upstream | in x | out x  | model | 建议来源 | 手动录入按钮    |
| ...      | ...  | ...    | ...   | ...      | ...            |
+--------------------------------------------------------------+
| 最近计费明细（请求级快照）                                  |
| time | model | upstream | tokens | base price | multiplier   |
| final cost | price source | status                            |
+--------------------------------------------------------------+
```
