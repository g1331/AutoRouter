...
- [ ] 2.1 在 `use-cliproxy.ts` 新增账号列表、账号同步、账号启停、账号字段更新的 hooks
- [ ] 2.2 在页面新增选中实例状态，选中后于实例表格下方内联面板展示账号列表
- [ ] 2.3 新增账号列表组件 `src/components/admin/cliproxy-accounts-table.tsx`，展示账号文件名、服务商、状态、模型数量、前缀，并提供行操作菜单
- [ ] 2.4 新增账号字段编辑弹窗，支持编辑前缀、出站代理、优先级、备注
- [ ] 2.5 实现账号同步与账号启停操作及其结果提示
- [ ] 2.6 为账号相关 hooks 与账号表格、账号编辑弹窗补测试
- [ ] 2.7 运行 `pnpm test:run`、`pnpm exec tsc --noEmit`、`pnpm lint` 确认通过，提交本阶段代码
...
- [ ] 4.1 在 `use-cliproxy.ts` 新增创建池上游与创建单账号映射上游的 hooks
- [ ] 4.2 在实例行操作菜单新增按服务商一键创建池上游入口，经确认弹窗执行
- [ ] 4.3 在账号行操作菜单新增将账号映射为上游入口，经确认弹窗执行
- [ ] 4.4 为上游创建 hooks 与创建入口补测试
- [ ] 4.5 运行 `pnpm test:run`、`pnpm exec tsc --noEmit`、`pnpm lint` 确认通过，提交本阶段代码
