...
- [x] 1.1 在 `schema-pg.ts` 和 `schema-sqlite.ts` 的 `upstreams` 表中新增 `spendingRules`（JSON, nullable）字段，对应 DB 列名 `spending_rules`
...
- [x] 2.1 创建 `src/lib/services/upstream-quota-tracker.ts`，实现 QuotaTracker 单例类，包含内存缓存结构 `Map<upstreamId, RuleCacheEntry[]>`（每条规则独立追踪）
...
- [x] 2.5 实现 `getQuotaStatus(upstreamId)` / `getAllQuotaStatuses()`，返回上游级状态 + 规则级状态（`currentSpending` / `spendingLimit` / `percentUsed` / `isExceeded` / `resetsAt` 等）
...
- [x] 3.3 在 `UpstreamSelectionResult` 中增加 `quotaFiltered` 字段，记录因限额被排除的上游数量
...
- [x] 5.1 在 `upstream-crud.ts` 的 `UpstreamCreateInput` / `UpstreamUpdateInput` / `UpstreamResponse` 接口中新增 `spendingRules` 字段（对应 API 字段 `spending_rules`）
- [x] 5.2 在 `upstream-crud.ts` 的 `createUpstream` / `updateUpstream` / `formatUpstreamResponse` 中处理 `spendingRules` 的读写与转换
- [x] 5.3 在 `src/app/api/admin/upstreams/route.ts` 和 `[id]/route.ts` 的 Zod schema 中新增限额字段校验逻辑（rolling 类型必须提供 hours、limit 必须为正数等）
- [x] 5.4 新建 `src/app/api/admin/upstreams/quota/route.ts`，实现 `GET` 端点返回所有有限额上游的消费状态
- [x] 5.5 为上游 CRUD API 的限额字段和 quota 端点编写单元测试

## 6. 前端类型与 Hooks

- [x] 6.1 在 `src/types/api.ts` 的 `UpstreamCreate` / `UpstreamResponse` 中新增 `spending_rules` 字段类型定义
- [x] 6.2 在 `src/types/api.ts` 中新增 `UpstreamQuotaStatus` 接口类型
- [x] 6.3 在 `src/hooks/use-upstreams.ts` 中新增 `useUpstreamQuota()` query hook，调用 quota API

## 7. 前端表单

- [x] 7.1 在 `upstream-form-dialog.tsx` 的 Zod schema 中新增限额规则校验（`spending_rules[].period_type`、`spending_rules[].limit`、`spending_rules[].period_hours`）
