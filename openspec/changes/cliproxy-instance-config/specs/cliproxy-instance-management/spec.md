...
系统 SHALL 按实例运行模式对地址执行差异化校验。受管 sidecar 模式下系统 SHALL 仅校验地址为格式合法的 `http` 或 `https` URL，并允许私有与内网地址。外部服务模式下系统 MUST 对地址执行完整 SSRF 校验，拦截私有 IP、回环地址、链路本地地址与云元数据端点。地址校验 SHALL 同时作用于代理转发基础地址与管理 API 地址。
