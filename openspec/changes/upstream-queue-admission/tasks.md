...
- [x] 4.3 更新日志表格、紧凑视图与路由决策时间线，显示等待状态标识、等待终止链路和真实候选熔断状态

## 5. 测试与验收

- [x] 5.1 为准入服务和并发选择逻辑补齐单元测试，覆盖即时成功、FIFO 恢复、队列上限、等待超时、客户端断开和保留移交顺序
- [x] 5.2 为代理路由与管理端组件补齐测试，覆盖等待恢复执行、单次重选、流式释放、表单回显、紧凑视图标识和时间线展示
- [x] 5.3 运行 `pnpm lint`、`pnpm exec tsc --noEmit`、`pnpm test:run` 与 `pnpm db:check:consistency`，确认本次变更达到可提交状态
