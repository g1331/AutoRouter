...
- [x] 9.1 新增 `playwright.visual.config.ts`（复用 e2e 的 SQLite + dev server bootstrap，visual + a11y 两 project）+ package.json `test:visual`/`test:a11y` 脚本；清理 `tests/visual/archive`（删前核实引用）（archive 实际不存在——Test-Path False 且 git log 无记录，无需清理；新增共享 mock `tests/support/admin-page-mocks.ts`，两 spec 重写为四页 × 双主题）
- [x] 9.2 视觉基线一次性重生成（`--update-snapshots`，login/dashboard/keys/upstreams 四张 fullPage，注记生成平台 Windows）；axe AA 全量复跑（text-dim/bronze 命中正文则按 D3 兜底档提亮）（首跑 admin 三页 500 系 Turbopack 持久化缓存损坏，删 .next 重启后复现消失；axe 实测 4 项兜底修复：①亮色状态四色整档加深 `#106b3f`/`#7d490a`/`#a53228`/`#325a7d`（原值在 12% muted 合成底上 3.8–4.5:1 不达标）②sidebar System 分组标签去 `/60` 透明度③upstreams 两个筛选 SelectTrigger 补 aria-label（combobox 不能从内容取名）④对比度单测新增 8 项 muted-over-surface-3 最坏合成底断言；修复后 a11y 8/8、visual 基线 4 张生成且复跑稳定、亮色双页浏览器观感核验正常）
- [x] 9.3 全量验证：`pnpm test:run --coverage`、`pnpm e2e`、`pnpm lint`、`pnpm format:check`、`pnpm exec tsc --noEmit`、`pnpm build`（DB_TYPE=postgres）（覆盖率跑 209 文件 3162 passed | 1 skipped，Statements 81.99%；e2e 11/11 8.3s；lint/format/tsc 0 错误；postgres build 成功退出 0）
- [x] 9.4 push 分支 + 建 PR（附四页前后截图与双主题对照，模板齐全）；CI 绿后停在交接边界（合并由用户决定）（PR #223；正文按模板齐全，Screenshots 引用入库的暗色四页基线 raw URL，亮色以 axe 双主题扫描 + 浏览器逐页核验交代）
