...
### Decision 4: UI 采用“单行横向步骤轨道 + 模式化阶段切分”

**Choice:** 默认不展开，单行展示阶段与耗时；非流式请求保持“决策、请求、响应、完成”，流式请求将输出过程拆分为“请求、首个输出、生成、完成”，并允许用 `0ms` 保留仅承担顺序语义的阶段锚点。
...
```text
[非流] [决策 18ms]──[请求 42ms]──[响应 0ms]──[完成 200 · 42ms]
[流式] [决策 18ms]──[请求 0ms]──[首个输出 338ms (+320ms)]──[生成 1.1s (+740ms)]──[完成 200 · 1.1s]
...
| 层级 | 内容 | 说明 |
|---|---|---|
| L1 | 阶段主标签（按请求模式在决策/请求/响应/完成与决策/请求/首个输出/生成/完成之间切换） | 首屏可读，固定顺序 |
