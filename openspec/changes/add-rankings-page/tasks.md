...
- [ ] 1.1 重构 `getLeaderboardStats`：将四个维度的聚合拆为按维度的查询函数，旧签名与响应结构不变，现有单测保持通过
- [ ] 1.2 各维度指标拉齐到 7 项：费用聚合并入主查询（LEFT JOIN request_billing_snapshots，确认 1:1 后移除二次费用查询），为缺失维度补 TTFT/TPS/缓存命中聚合
- [ ] 1.3 新增错误率聚合（非 2xx 已完成 / 已完成，排除 status_code 为空的进行中日志）
- [ ] 1.4 实现单维度查询入口：`dimension` + `sort_by`（SQL 层 ORDER BY），非法值报错
- [ ] 1.5 实现 `compare` 环比：上一等长窗口主聚合，返回 prev_rank / prev_request_count；custom 范围窗口前移等长
- [ ] 1.6 stats-service 单测：错误率口径、排序正确性、环比窗口计算、向后兼容（不传 dimension 时结构不变）
