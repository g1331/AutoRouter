...
- [ ] 4.1 重构 `src/app/[locale]/(dashboard)/dashboard/page.tsx` 与 `src/components/dashboard/*`（`stats-cards`、`usage-chart`、`leaderboard-section`、`time-range-selector`、`chart-theme`）以匹配新视觉语义。
- [ ] 4.2 为 Dashboard 图表实现明暗双套配色方案，并验证两套方案都不使用炫目蓝紫主导背景。
