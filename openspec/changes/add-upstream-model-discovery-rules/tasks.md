...
- [ ] 4.1 在 proxy 主链路中接入显式模型规则过滤，将其放在路径能力匹配之后、负载均衡选择之前，并保证 exact / regex / alias 未命中时不会继续保留该上游候选。
- [ ] 4.2 补充模型规则过滤相关的日志与观测字段，确保能够区分“能力不匹配”“模型规则未命中”“健康/配额/并发排除”等候选淘汰原因。
- [ ] 4.3 完成模型发现、目录导入、严格候选过滤与 fallback 来源标记的单元/接口/关键 UI 验证，并跑通本次变更涉及的质量门禁。
