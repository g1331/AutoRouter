...
当前 Gemini 客户端请求已经命中网关路径能力，但在入站鉴权阶段被提前拒绝，导致请求无法转发到上游，后续 token 统计链路也无法生效。与此同时，现有 usage 解析尚未覆盖 Gemini `usageMetadata` 与 Anthropic `cache_creation` TTL 细分字段，造成日志与成本分析的观测盲区。
...
- 代理入口与鉴权：`src/app/api/proxy/v1/[...path]/route.ts`、`src/lib/utils/auth.ts`
