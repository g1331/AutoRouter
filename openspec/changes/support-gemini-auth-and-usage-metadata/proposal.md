...
当前 Gemini 客户端请求已经命中网关路径能力，但在入站鉴权阶段被提前拒绝，导致请求无法转发到上游，后续 token 统计链路也无法生效。与此同时，现有 usage 解析尚未覆盖 Gemini `usageMetadata` 与 Anthropic `cache_creation` TTL 细分字段，造成日志与成本分析的观测盲区。并且，Gemini CLI 原生路径请求（`/v1beta/models/{model}:...`）的模型名来自 URL 路径而不是 body，现有实现只读取 `body.model`，导致请求日志与 billing snapshot 的 model 为空。
