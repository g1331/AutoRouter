...
- [ ] 5.1 新增 CLI OAuth 管理入口，展示 CLIProxyAPI 服务连接、management 状态、出站代理和最近测试结果
- [ ] 5.2 实现 OAuth 账号列表，展示 provider、账号名称、启用状态、模型数量、冷却或错误状态和可用操作
- [ ] 5.3 实现 Codex、Claude、Gemini OAuth 登录入口，展示授权 URL、device code、过期时间和轮询状态
- [ ] 5.4 在上游创建弹窗加入 CLI OAuth 上游预设入口，允许保存前修改预填配置
- [ ] 5.5 支持从账号列表打开固定账号上游创建流程，并回显账号 prefix 与模型规则初始值
- [ ] 5.6 补齐中英文文案、加载态、空态、失败态、选择反馈和移动端线性布局
- [ ] 5.7 补充组件和 hooks 测试，覆盖连接配置、账号列表、OAuth 登录状态、预设填充和固定账号创建
