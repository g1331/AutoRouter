...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const result = await refreshStoredUpstreamModelCatalog(id);
    return NextResponse.json({
      upstream: transformUpstreamToApi(result.upstream),
      refresh: {
    ...
    log.error({ err: error }, "failed to refresh upstream model catalog");
