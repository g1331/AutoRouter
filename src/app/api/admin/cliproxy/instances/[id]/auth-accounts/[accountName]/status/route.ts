...
import { setCliproxyAuthAccountStatus } from "@/lib/services/cliproxy-auth-account-service";
...
/**
 * PATCH /api/admin/cliproxy/instances/:id/auth-accounts/:accountName/status - 启停账号。
 */
export async function PATCH(request: NextRequest, context: RouteContext): Promise<Response> {
  ...
  try {
    const account = await setCliproxyAuthAccountStatus(
      id,
    ...
    log.error({ err }, "Failed to update CLIProxyAPI auth account status");
