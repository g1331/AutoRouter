...
| `503 {error:{code:"ALL_UPSTREAMS_UNAVAILABLE", reason:"UPSTREAM_CIRCUIT_OPEN"}}` | `load-balancer.ts`、`route.ts`           | 所有候选被熔断拦截。`user_hint` 会带被熔断的上游名与预计恢复秒数，routing_decision.excluded 里有 `circuit_open` 条目。检查上游服务 / 密钥余额，或在管理后台手动关闭熔断 |
