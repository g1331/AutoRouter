...
| `503 {error:{code:"ALL_UPSTREAMS_UNAVAILABLE", reason:"UPSTREAM_CIRCUIT_OPEN"}}` | `load-balancer.ts`、`route.ts`           | 所有候选被熔断拦截。`user_hint` 只带最快恢复秒数（各候选剩余时间的最小值，上游身份不下发），被熔断的具体上游看 routing_decision.excluded 的 `circuit_open` 条目。检查上游服务 / 密钥余额，或在管理后台手动关闭熔断。例外：`GET /v1/models` 在该场景下会回退本地模型目录返回 200 |
| `503 {error:{code:"ALL_UPSTREAMS_UNAVAILABLE", reason:"NO_HEALTHY_CANDIDATES"}}` | `load-balancer.ts`、`:243-303`           | 所有 tier 遍历完毕但非熔断导致（如 quota 耗尽、显式排除）。看每个上游的 circuit_breaker_states 与上游绑定的 quota                                                                                                                                                               |
