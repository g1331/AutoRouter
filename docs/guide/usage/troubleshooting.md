...
| `503 {error:{code:"ALL_UPSTREAMS_UNAVAILABLE", reason:"UPSTREAM_CIRCUIT_OPEN"}}` | `load-balancer.ts`、`route.ts`           | 所有候选被熔断拦截。`user_hint` 只带预计恢复秒数（上游身份不下发），被熔断的具体上游看 routing_decision.excluded 的 `circuit_open` 条目。检查上游服务 / 密钥余额，或在管理后台手动关闭熔断。例外：`GET /v1/models` 在该场景下会回退本地模型目录返回 200 |
