...
密钥管理页对每条 Key 提供四类生命周期操作：

| 操作        | 行为                                                                                    |
| ----------- | --------------------------------------------------------------------------------------- |
| 编辑        | 修改名称、访问模式、绑定上游、模型白名单、过期、消费规则                                |
| 停用 / 启用 | 切换 `enabled` 状态，停用后鉴权会被拒绝。可恢复                                         |
| 撤销        | 立即失效该 Key 的鉴权能力，保留记录与历史日志便于追溯。不可恢复为可用状态               |
| 删除        | 从数据库移除该 Key 记录。历史日志中该 Key 的相关字段会保留以便追溯，但 Key 本身不可恢复 |

撤销和删除的区别：撤销保留记录但永久失效，删除则把记录抹掉。多数情况下应当用「撤销」而不是「删除」，便于历史日志可追溯。
