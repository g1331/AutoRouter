...
AutoRouter 是一个 Next.js 全栈应用：同一个进程同时承担「管理后台 UI」「管理 API」「面向调用方的 AI 代理 API」三类职责，没有独立的后端服务。所有持久化状态落在一个关系型数据库（默认 PostgreSQL，可选 SQLite），所有运行期决策（鉴权、选路、熔断、计费、录制）都基于该数据库的当前快照做出。本页给出整套系统的分层结构、关键模块以及彼此之间的关系，让阅读者在动手改任何具体功能前先建立全貌。
...
- `CORS_ORIGINS` 是逗号分隔的白名单，默认 `http://localhost:3000`。
