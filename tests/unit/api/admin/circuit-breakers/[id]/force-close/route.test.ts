...
import { POST } from "@/app/api/admin/circuit-breakers/[id]/force-close/route";
...
describe("POST /api/admin/circuit-breakers/{upstreamId}/force-close", () => {
  const mockUpstream = {
    id: "upstream-1",
  ...
  it("should return 401 for invalid auth", async () => {
    const request = new Request(
      "http://localhost/api/admin/circuit-breakers/upstream-1/force-close",
      {
        method: "POST",
  ...
  it("should return 404 when upstream does not exist", async () => {
    ...
    const request = new Request(
      "http://localhost/api/admin/circuit-breakers/non-existent/force-close",
      {
        method: "POST",
  ...
  it("should force close circuit breaker successfully", async () => {
    vi.mocked(db.query.upstreams.findFirst).mockResolvedValue(mockUpstream);
    vi.mocked(forceClose).mockResolvedValue(undefined);
    ...
    expect(data).toMatchObject({
      success: true,
      message: "Circuit breaker forced to CLOSED for upstream 'OpenAI Upstream'",
      upstream_id: "upstream-1",
      upstream_name: "OpenAI Upstream",
      action: "force_close",
    });

    // Verify forceClose was called with correct upstream ID
    expect(forceClose).toHaveBeenCalledWith("upstream-1");
    expect(forceClose).toHaveBeenCalledTimes(1);
  });

  it("should handle forceClose errors gracefully", async () => {
    vi.mocked(db.query.upstreams.findFirst).mockResolvedValue(mockUpstream);
    vi.mocked(forceClose).mockRejectedValue(new Error("Failed to force close"));
